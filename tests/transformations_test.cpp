...
BOOST_AUTO_TEST_CASE ( jacobi_rotation_matrix ) {
    ...
    Eigen::MatrixXd J = libwrp::jacobi_rotation_matrix(0, 1, boost::math::constants::half_pi<double>(), 2);
...
BOOST_AUTO_TEST_CASE ( analytical_jacobi_one_electron_toy ) {
    ...
    Eigen::MatrixXd U = libwrp::jacobi_rotation_matrix(P, Q, theta, 2);
    BOOST_CHECK(libwrp::rotate_integrals(h, U).isApprox(libwrp::rotate_one_electron_integrals_jacobi(h, P, Q, theta)));

    // Test with another angle theta
    theta = 7.09214;
    U = libwrp::jacobi_rotation_matrix(P, Q, theta, 2);
    BOOST_CHECK(libwrp::rotate_integrals(h, U).isApprox(libwrp::rotate_one_electron_integrals_jacobi(h, P, Q, theta)));
}
...
BOOST_AUTO_TEST_CASE ( analytical_jacobi_one_electron_h2o ) {
    ...
    Eigen::MatrixXd U = libwrp::jacobi_rotation_matrix(P, Q, theta, basis.nbf());
    BOOST_CHECK(libwrp::rotate_integrals(T, U).isApprox(libwrp::rotate_one_electron_integrals_jacobi(T, P, Q, theta)));
}
